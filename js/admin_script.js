...
const STORAGE_PRODUCTS_VERSION_KEY = "IndahBeauteProductsVersion";
const CURRENT_PRODUCTS_VERSION = "2026-07-04-24";
...
function loadData() {
  ...
  const storedVersion  = safeGetItem(STORAGE_PRODUCTS_VERSION_KEY);
  const shouldUseDefaultProducts = !storedProducts.length || storedVersion !== CURRENT_PRODUCTS_VERSION;
  ...
  if (shouldUseDefaultProducts && defaultList.length) {
    products = defaultList.map(item => normalizeProductRecord({
      ...item,
      category: item.category,
      foto: item.foto || item.image || item.img || null,
      image: item.image || item.foto || item.img || null
    }, item.id));
    safeSetItem(STORAGE_PRODUCTS_VERSION_KEY, CURRENT_PRODUCTS_VERSION);
    saveProducts();
  } else if (!normalizedStoredProducts.length && defaultList.length) {
  ...
  if (defaultList.length && !shouldUseDefaultProducts) {
...
function initEditProduk() {
  ...
  const categorySelect = document.getElementById("epKategori");
  if (categorySelect) {
    categorySelect.value = getCategoryLabel(p.category) || "Skincare";
  }
  document.getElementById("epDesc").value     = p.desc || "";
  document.getElementById("epHot").checked    = p.hot || false;
  const emojiInput = document.getElementById("epEmoji");
  ...
  async function doEdit() {
    ...
    const emojiValue = document.getElementById("epEmoji")?.value?.trim() || "";
    if (emojiValue) p.emoji = emojiValue;
